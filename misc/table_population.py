...
for i in range(1, 91):
    ...
    r = requests.post('http://localhost:9000/users/register', headers=headers, json=user)
    campo = {'idJefeCampo':{'username':user['username']}, 'region':'CARIBE', 'pozos':[]}
    r = requests.post('http://localhost:9000/campo', headers=headers, json=campo)
    _id = r.json()['id']
    for j in range(0, 14):
        p = {'lon':34.66,'lat':-72.123123,'estado':'ABIERTO','campo':{'id':_id}}
        r = requests.post('http://localhost:9000/pozo', headers=headers, json=p)
